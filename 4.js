...
// for that we have 'v-for=""';;; here if we want index like 'key' so use it as 
// v-for='(x, index) in foods' key='index' <-- how do we make it expression use v-bind ;; ;so by this it will be not string the value will be rendered in it

// next we have conditionals rendering -- ****************** for that we have other directives called
// v-if and v-show;;


// now what about fns, how we can use fns in vue:::::::::::::::####
// so for that we have another property (attribute) which is known as --> 'methods'
// so insite it we defined our fns and methods
// how to it in our html ; so for that we have another directive called -> 'v-on: takes multiple options (click event or huber event'
//  mostly we use 'click'event ;; v-on:click='do_this_whne click event will be occured'
// so for this 'this' we don't use arrow fn so this willl be not refer 'data';; bz this will be refer window object in case of arrow fn


// note:: vvvim when ever dependent data property are changed , the computed property re-run and try get all it's value 
// that is vanifit of making 'computed' property not making these property in 'methods' 
